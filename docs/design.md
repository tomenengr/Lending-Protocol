...
当前版本已经包含 USDC base asset pool、基于 index 的利息累计、utilization-based kink rate model、supply cap、global borrow cap、isolation mode、全局 pause、资产 freeze、协议储备金、储备金提取、坏账记录、坏账再资本化，以及 Compound III 风格的 `absorb / buyCollateral` 两阶段清算路径。但仍有意不做多借款资产、aToken/cToken、闪电贷、治理、可升级代理和前端。项目重点是把风险控制主链路、资金池会计、利息累计和清算账务做完整、测清楚、讲明白。
...
- 把 10% 借款利息计入协议储备金。
- 允许 owner 提取已落账且有 USDC cash 支持的协议储备金。
...
- 支持 permissionless recapitalization，允许外部资金注入 USDC 修复坏账。
...
借款利息中的 10% 会进入 `protocolReservesUSDC`。这部分储备金有两个用途：

- 先于 `badDebtUSDC` 吸收 `absorb` 产生的坏账。
- 在有实际 USDC cash 支持时，由 owner 通过 `withdrawReserves` 提取。

`withdrawReserves` 会先调用 `accrueInterest()`，然后检查：
...
提取时同时减少 `protocolReservesUSDC` 和协议 USDC cash，因此 `getAvailableLiquidity()` 不会因为 reserve withdraw 下降。换句话说，owner 只能提取协议收入，不能绕过供应者的流动性边界。

## 坏账再资本化

当 `absorb` 后抵押品折价可回收价值不足以覆盖债务，协议会记录 `badDebtUSDC`。`recapitalizeBadDebt(amountUSDC)` 允许任何地址注入 MockUSDC 降低坏账：
...
这个入口是 permissionless 的，因为修复坏账对协议和供应者都是正向操作。多传金额不会被多扣，协议只收当前剩余坏账规模。

## 测试策略

测试按行为拆分：

- Unit tests：oracle、price heartbeat、deposit、supply、borrow、repay、withdraw、liquidation、absorb、buyCollateral、caps、isolation、pause、freeze、reserve withdraw、bad debt recapitalization、interest、rate。
