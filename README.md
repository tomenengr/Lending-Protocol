...
- WBTC 启用 isolation mode，不能和其他抵押品混用，且有独立 debt ceiling。
- owner 可以执行全局 pause，暂停新增风险和资金流出。
- owner 可以对单个抵押资产执行 freeze，阻止该资产新增抵押和基于该资产继续借款。
...
- Oracle 支持默认 stale period 和单资产 heartbeat override，用于控制不同价格源的有效期。
