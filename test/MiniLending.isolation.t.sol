...
contract MiniLendingIsolationTest is MiniLendingTestBase {
    function test_isolatedCollateralCapsBorrowableAndDebt() public {
    ...
    function test_nonIsolatedCollateralUsesNormalBorrowableValue() public {
    ...
    function test_isolationRejectsMixingCollateralTypes() public {
        _depositWbtc(alice, 1e8);
        vm.startPrank(alice);
    ...
    function test_canAddSameIsolatedCollateralOrSwitchAfterWithdraw() public {
        _depositWbtc(alice, 1e8);
        _depositWbtc(alice, 1e8);
        assertEq(lending.collateralBalance(alice, address(wbtc)), 2e8);

        vm.prank(alice);
        lending.withdrawCollateral(address(wbtc), 2e8);
